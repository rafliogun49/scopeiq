...
function ProjectDetailContent({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10">
      ...
      <div className="mb-8 overflow-hidden rounded-[2rem] border border-slate-200/70 bg-white/90 shadow-[0_28px_80px_-45px_rgba(15,23,42,0.45)]">
        <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div className="p-6 md:p-7">
            <p className="mb-3 font-geist text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">
              Project dossier
            </p>
            <h1 className="max-w-3xl font-geist text-4xl font-semibold leading-tight tracking-tight text-slate-950">
              {project.name}
            </h1>
            <p className="mt-4 max-w-2xl font-satoshi text-sm leading-relaxed text-slate-600">
              {project.idea}
            </p>
          </div>
          <div className="border-t border-slate-200/70 bg-slate-50/70 p-6 lg:border-l lg:border-t-0">
            <div className="grid gap-4">
              <div>
                <p className="font-geist text-xs font-medium uppercase tracking-[0.14em] text-slate-500">
                  Created
                </p>
                <p className="mt-1 font-geist text-lg font-semibold tracking-tight text-slate-950">
                  {createdDate}
                </p>
              </div>
              <div>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
        <Card className="rounded-[2rem] border border-slate-200/70 bg-white/90 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.38)]">
          ...
          <CardContent className="px-6 py-6">
            <p className="font-satoshi text-[0.96rem] leading-7 text-slate-600">
              {project.idea}
            </p>
          </CardContent>
        </Card>

        <Card className="rounded-[2rem] border border-slate-200/70 bg-white/80">
          <CardHeader className="border-b border-slate-200/70 px-6 pb-5 pt-6">
            <CardTitle className="font-geist text-lg font-semibold">
              Known competitors
            </CardTitle>
            ...
            project.known_competitors.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {project.known_competitors.map((competitor) => (
                  <span
                    key={competitor}
                    className="rounded-full bg-slate-100 px-3 py-1.5 text-sm font-satoshi font-medium text-slate-700 ring-1 ring-slate-200/70"
                  >
                    {competitor}
                  </span>
                ))}
              ...
              <p className="font-satoshi text-sm text-slate-500">
      ...
      <div className="mt-8 rounded-[2rem] border border-slate-200/70 bg-white/90 p-5 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.38)]">
        <h2 className="mb-4 font-geist text-lg font-semibold">Next Steps</h2>
        <div className="grid gap-4 md:grid-cols-[minmax(0,1.4fr)_minmax(280px,0.8fr)]">
          <Link to="/projects/$projectId/runs/new" params={{ projectId }}>
            <Card className="h-full cursor-pointer rounded-[1.5rem] border border-emerald-200/70 bg-emerald-50/70 shadow-none transition-transform hover:-translate-y-0.5">
          ...
          <div className="divide-y divide-slate-200 rounded-[1.5rem] border border-slate-200/70 bg-white">
            <Link
